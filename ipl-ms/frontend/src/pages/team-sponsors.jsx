...
export default function TeamSponsorsPage() {
  ...
  const [formData, setFormData] = useState({ teamId: "", sponsorId: "" });
  const [submitting, setSubmitting] = useState(false);

  const fetchData = async () => {
    try {
      const [tsRes, tRes, sRes] = await Promise.all([
        fetch("/api/team-sponsors"),
        fetch("/api/teams"),
        fetch("/api/sponsors"),
      ]);
      const [tsData, tData, sData] = await Promise.all([
        tsRes.json(),
        tRes.json(),
        sRes.json(),
      ]);
      if (tsData.success) setTeamSponsors(tsData.data || []);
      if (tData.success) setTeams(tData.data || []);
      if (sData.success) setSponsors(sData.data || []);
    } catch (err) {
  ...
  const sponsorMap = Object.fromEntries(sponsors.map((s) => [s._id, s.name]));

  return (
    <div>
      <div style={styles.header}>
        <h2 style={styles.title}>Team-Sponsors</h2>
        <button
      ...
      {showForm && (
        <div style={styles.modalOverlay} onClick={() => setShowForm(false)}>
          <div style={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            <h3 style={styles.modalTitle}>Attach Sponsor to Team</h3>
            <form onSubmit={handleSubmit} style={styles.form}>
              ...
              <div style={styles.formGroup}>
                ...
                <select
                  ...
                  style={styles.select}
                >
                  <option value="" style={{ backgroundColor: 'var(--mdb-darker)' }}>Select Sponsor</option>
                  {sponsors.map((s) => (
                    <option key={s._id} value={s._id} style={{ backgroundColor: 'var(--mdb-darker)' }}>
                      {s.name}
                    </option>
                  ))}
                </select>
      ...
      {teamSponsors.length === 0 ? (
        <div style={styles.emptyState}>
          <p style={styles.emptyTitle}>No team-sponsor links found</p>
        </div>
      ) : (
          ...
          <table style={styles.table}>
            ...
            <tbody>
              {teamSponsors.map((ts) => (
                ...
                >
                  <td style={styles.tableCell}>{teamMap[ts.teamId] || ts.teamId}</td>
                  <td style={styles.tableCell}>{sponsorMap[ts.sponsorId] || ts.sponsorId}</td>
                </tr>
              ))}
            </tbody>
          </table>
